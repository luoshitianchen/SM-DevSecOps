# SM-DevSecOps 容灾恢复手册（DR RUNBOOK）

> 服务：sm-devsecops（DevSecOps 服务）｜ RTO 目标：≤ 15 分钟 ｜ RPO 目标：≤ 5 分钟
...
kubectl scale deployment/sm-devsecops -n sm-prod --replicas=0

# 3. 选择恢复点（RPO ≤ 5min）
#    - 全量备份：/backup/postgres/sm_devsecops_YYYYMMDD.dump
#    - WAL 归档：/archive/wal/
...
# 5. 验证数据完整性
psql -h <recovered-host> -U postgres -d sm_devsecops -c "SELECT count(*) FROM <核心表>;"

# 6. 恢复应用流量
kubectl scale deployment/sm-devsecops -n sm-prod --replicas=3
