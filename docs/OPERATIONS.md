# SM-DevSecOps 运维手册（OPERATIONS）

> 服务：sm-devsecops（DevSecOps 服务）｜ 端口：8017

## 1. 服务概述与依赖

SM-DevSecOps 是 SM 数据平台微服务集群中的 **DevSecOps 服务** 服务。

**依赖组件：**
- PostgreSQL 主库（`sm_devsecops` 数据库）
- SM-IAM（统一身份认证）
...
2. **回滚命令**：
   ...
   # 或指定版本
   helm rollback sm-devsecops <REVISION> -n sm-prod
   ```
3. **RTO 目标**：≤ 5 分钟。
4. **验证步骤**：
...
| 操作 | 命令/步骤 | 频率 |
|---|---|---|
| 查看 Pod 状态 | `kubectl get pods -n sm-prod -l app.kubernetes.io/name=sm-devsecops` | 日常 |
| 查看日志 | `kubectl logs -n sm-prod -l app.kubernetes.io/name=sm-devsecops --tail=100` | 日常 |
| 进入 Pod 排查 | `kubectl exec -it <pod> -n sm-prod -- /bin/sh` | 按需 |
| 手动扩缩容 | `kubectl scale deployment/sm-devsecops -n sm-prod --replicas=5` | 紧急 |
| 查看 HPA | `kubectl get hpa -n sm-prod -l app.kubernetes.io/name=sm-devsecops` | 日常 |
